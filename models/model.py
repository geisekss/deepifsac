...
class RowColTransformer(nn.Module):
    def __init__(self, num_tokens, dim, nfeats, depth, heads, dim_head, attn_dropout, ff_dropout,style='col'):
        ...
        self.style = style
        self.dense_merge = nn.Linear(2*dim, dim)
        for _ in range(depth):
            if self.style in ['colrow', 'rowcol', 'parallel', 'colrowatt']:
                print('style is col --> row')
    ...
    def forward(self, x, x_cont=None, mask = None):
        ...
        # print(x.shape)
        _, n, _ = x.shape
        if self.style == 'colrow' or self.style == 'colrowatt':
        ...
        elif self.style == 'parallel':
            for attn1, ff1, attn2, ff2 in self.layers: 
                between_x = attn1(x)
                between_x = ff1(between_x)

                reshaped_x = rearrange(x, 'b n d -> 1 b (n d)')
                self_att_x = attn2(reshaped_x)
                self_att_x = ff2(self_att_x)
                self_att_x = rearrange(self_att_x, '1 b (n d) -> b n d', n = n)

                merged_x = torch.cat((between_x, self_att_x), dim=-1)

                # dense_layer = self.dense_merge(merged_x.shape[-1], n)

                # Apply the dense layer
                x = self.dense_merge(merged_x)
...
class TabAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        elif attentiontype in ['row','colrow', 'rowcol','parallel'] :
